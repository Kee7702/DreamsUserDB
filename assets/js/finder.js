...
(async()=>{const res=await fetch(fetchURL);const json=await res.json();importlist=(json);imports=new URLSearchParams(window.location.search).get('start') - 0;(async()=>{const res=await fetch(`https://raw.githubusercontent.com/Kee7702/DreamsUserDB/database/users`);const json=await res.json();userdb=(json);
document.getElementById('imp').onerror = function onerror(event) {
    this.src = 'https://assets.indreams.me/images/users/default.png'
}
...
if (!importlist[1] || !importlist[0].hasImports) {document.getElementById('audioClipsContainer').remove()}
