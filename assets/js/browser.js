...
(async()=>{
    ...
    (async()=>{
        ...
        !params.get("id") && !params.get("name")) {
            ...
            do {
                imports++,
                document.getElementById("audioClips").innerHTML+=`<div style="margin:4px;padding:10px;display:inline-grid;text-align:center;background:${importlist[imports].img[0].color};border-radius:10px;max-width:calc(100% - 28px);width:480px;height:120px;">
  <img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsMAAA7DAcdvqGQAAAALSURBVAgdY2BgAAAAAwABT0gKrwAAAABJRU5ErkJggg==" style="transform:scale(1.10345);height:50px;width:50px;position: absolute;background-image:url(&quot;/assets/images/cardOverlay.png&quot;), url(&quot;https://cdn-prod.puprod.indreams.me/${importlist[imports].img[0].hash}_2&quot;), url(&quot;https://cdn-prod.puprod.indreams.me/${importlist[imports].img[0].hash}_7&quot;);-webkit-mask-image:url(&quot;/assets/images/cardMaskImage.png&quot;);-webkit-mask-size:cover;border-radius:50%;background-color:${importlist[imports].img[0].color};background-size:cover;background-position:centercenter;">
  <a class="global-post-label"${importlist[imports].browse_id&&' href="/imports?id='+importlist[imports].browse_id+'"'} style="font-size:16px;line-height:27px;color:#CCC;margin-left:54px;margin-top:18px;font-family:Poppins;">${importlist[imports].import_name}</a>
  <img class="global-post-icon" src="https://cdn-prod.puprod.indreams.me/${userdb[importlist[imports].userID].imphash}_10" onerror="this.src='https://assets.indreams.me/images/users/default.png'" style="height: 20px; width: 20px; line-height: 20px; transform: scale(2) rotate(-5deg);">
  <a class="global-post-details" style="margin-left: 54px;position: absolute;margin-top: 4px;">${importlist[imports].import_date}</a>
  ${importlist[imports].source&&'<div style="align-self:flex-end"><audio src="https://audio.dreams.kee7702.tk/'+importlist[imports].source+'.mp3" controls="" preload="none" loop="" style="width:100%;margin:4px 0"></audio></div>'||'<a style="padding: 12px;background: #fff2;font-size:16px;line-height: 30px;color:#CCC;font-family:Poppins;border-radius: 16px;height: 30px;align-self: flex-end;margin: 4px 0;">No Audio File</a>'}
        ...
        if (params.get("id") || params.get("name")) {
            ...
            do {
                ...
                if (importlist[imports].browse_id == params.get("id") || importlist[imports].import_name == params.get("name") || ((parseInt(importlist[imports].import_name.replaceAll(' ', ''), 36)) + parseInt(importlist[imports].creation_id.split('/')[importlist[imports].creation_id.split('/').length - 1], 36)).toString(16).slice(0, 15) + importlist[imports].img[0].hash.slice(0, 5) == ('' + params.get("id")).slice(0, 20)) {
                    ...
                    b.style = "background:#000;margin-top:8px",
                ...
                importlist[imports + 1] || "" != document.getElementById("audioClips").innerHTML || (document.getElementById("audioClips").innerHTML = '<div class="global-post-container" style="background:#000;margin-top:8px"><a>404 Not Found<br>An import with the specified ID' + decodeURI(" %27" + params.get("id") + "%27 ") + "was not found in the database.</a></div>")
    ...
    (async()=>{
        ...
        (async()=>{
            ...
            document.getElementsByClassName("navigation-container")[2] && document.cookie.split("; ").find(a=>a.startsWith("limit="))?.split("=")[1] - 0 < 0 && (document.cookie = "limit=45; expires=" + new Date(new Date().valueOf() + 31536e6).toGMTString()),
            document.getElementById("audioImports").innerHTML = "Audio Imports"),
            document.getElementById('audioClips').innerHTML = `<div class="global-post-container" style="background: rgba(51, 51, 51, 0.733);"><a class="global-post-label" style="font-size: 16px;color: rgb(204, 204, 204);margin-top: 3px;">Compiling Audio Imports</a><a class="global-post-icon" style="line-height: 20px;">loop</a></div>`,
